...
heterogeneity_load_steady_state(filename = hank_2a_ssj);
...
verbatim;
    ...
    [testFailed, result] = run_test('heterogeneity.solve functionality and SSJ validation', @() test_solve_functionality(M_, oo_, G_eq, false), testFailed);
    testResults = [testResults; result];

    % Test the solve routine with custom impulse responses (calls solve with custom impulses)
    [testFailed, result] = run_test('heterogeneity.solve with custom impulse responses', @() test_solve_with_custom_impulse_responses(M_, options_, oo_, x_hat_dash, G, J, F, curlyDs, curlyYs, a_i, b_i, a_pi, b_pi), testFailed);
    testResults = [testResults; result];
