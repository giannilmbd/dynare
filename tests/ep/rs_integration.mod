...
//currently disabled due to stroud not having a zero node
...
% extended_path(periods=10, order=2,integration=stroud_3);
% 
% if ~oo_.extended_path.status
%     error('Extended path did not find solution in rstrue.mod')
% end
% 
extended_path(periods=10, order=2,integration=stroud_5);

if ~oo_.extended_path.status
    error('Extended path did not find solution in rstrue.mod')
end

extended_path(periods=10, order=2,integration=gauss_hermite,nodes=3);
