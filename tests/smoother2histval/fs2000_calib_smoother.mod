// Test that smoother2histval works (with an outfile) after a calib_smoother command
...
calib_smoother(datafile=fsdat_simul);
...
assert(all(hf.y.data == oo_.SmoothedVariables.y(4:5)))
