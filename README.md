...
```sh
# Minimal packages
dnf install -y gcc gcc-c++ make gcc-gfortran boost-devel suitesparse-devel flex bison meson redhat-rpm-config
# Octave packages
...
```sh
pacman -S git make meson boost blas gcc-fortran gcc-libs
```
...
```sh
arch -$ARCH $BREWDIR/bin/brew install meson bison flex boost gcc veclibfort octave sphinx-doc docutils wget pkg-config git-lfs cmake
```
