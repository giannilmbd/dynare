function [deviations, baseline, irfs] = irf(M_,options_,oo_,initialcondition, innovationbaseline, listofshocks, listofvariables, varargin)
% [deviations, baseline, irfs] = backward_model_irf(M_,options_,oo_,initialcondition, innovationbaseline, listofshocks, listofvariables, varargin)
...
%
% OUTPUTS
% - deviations          [struct of dseries]             Deviation of
%                                                       simulation from baseline
% - baseline            [struct of dseries]             Baseline simulation
% - irfs                [struct of dseries]             Shock scenario simulation
%
% REMARKS
...
%   the associated paths for endogenous variables listed in the sixth input
%   argument.
% - If fifth argument is not empty, periods must not be greater than innovationbaseline.nobs.
...
for i=1:length(listofshocks)
    ...
    alldeviations = dseries(transpose(endo_simul__1-endo_simul__0), initialcondition.init, endonames(1:M_.orig_endo_nbr), M_.endo_names_tex(1:M_.orig_endo_nbr));
    if nargout>2
        allirfs = dseries(transpose(endo_simul__1), initialcondition.init, endonames(1:M_.orig_endo_nbr), M_.endo_names_tex(1:M_.orig_endo_nbr));
    end
...
if nargout>1
    baseline = dseries(transpose(endo_simul__0), initialcondition.init, endonames(1:M_.orig_endo_nbr), M_.endo_names_tex(1:M_.orig_endo_nbr));
