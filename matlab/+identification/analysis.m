...
% For Spectrum: transform into correlation-type matrices (as above with AHess)
    ...
    if ~options_ident.no_identification_moments && ~error_indicator.identification_moments
        ...
        if advanced
            ...
            [ide_moments.pars, ide_moments.cosndMOMENTS] = identification.bruteforce(M_.dname,M_.fname,dMOMENTS(ind_dMOMENTS,:), max_dim_cova_group, options_.TeX, options_ident.name_tex, options_ident.tittxt, tol_deriv,options_.console_mode);
        end
...
%% Perform identification checks, i.e. find out which parameters are involved
    if checks_via_subsets
        ...
        [ide_dynamic, ide_reducedform, ide_moments, ide_spectrum, ide_minimal] = ...
            identification.checks_via_subsets(ide_dynamic, ide_reducedform, ide_moments, ide_spectrum, ide_minimal, totparam_nbr, modparam_nbr, options_ident, error_indicator, options_.console_mode);
