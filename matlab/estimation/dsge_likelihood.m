    ...
    if (kalman_algo==3)
        ...
        if no_missing_data_flag
            [dLIK,dlik,a_0_given_tm1,Pstar] = kalman_filter_d(Y, 1, size(Y,2), ...
                                                  ...
                                                  kalman_tol, diffuse_kalman_tol, options_.presample, ...
                                                  T,R,QQ,H,Z,pp);
        else
...
if ((kalman_algo==1) || (kalman_algo==3)) || (kalman_algo == 5) % Multivariate Kalman Filter
    if no_missing_data_flag && ~options_.occbin.likelihood.status
        ...
        [LIK,lik] = missing_observations_kalman_filter(dataset_info.missing.aindex,dataset_info.missing.no_more_missing_observations,Y,diffuse_periods+1,size(Y,2), ...
                                                       a_0_given_tm1, Pstar, ...
...
if (kalman_algo==2) || (kalman_algo==4)
    ...
    [LIK, lik] = univariate_kalman_filter(dataset_info.missing.aindex,dataset_info.missing.no_more_missing_observations,Y,diffuse_periods+1,size(Y,2), ...
                                          a_0_given_tm1,Pstar, ...
                                          options_.kalman_tol, ...
                                          options_.riccati_tol, ...
                                          options_.presample, ...
                                          T,Q,R,H1,Z,mmm,pp,Zflag,diffuse_periods,analytic_deriv_info{:});
    if analytic_derivation
        LIK1=LIK;
        LIK=LIK1{1};
        lik1=lik;
...
elseif options_.estimate_initial_states_endogenous_prior
    if not(options_.lik_init==2 && options_.Harvey_scale_factor==0)
        error('Init state endogenous prior not supported without conditional likelihood')
    else
        [lnpriorendoinitstate, lnpriorinitstate]  = init_state_endogenous_prior(a_0_given_tm0,T,R,Q,xparam1,bayestopt_,options_);
...
% optionally second) order derivatives of the state-space system matrices
