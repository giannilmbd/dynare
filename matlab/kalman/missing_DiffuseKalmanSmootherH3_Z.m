...
if isoccbin
    ...
    if occbin_options.opts_simul.waitbar && first_period_occbin_update<smpl
        [hh_fig, length_of_old_string] = wait_bar.run(0,[],'OccBin: Piecewise Kalman Filter', options_.console_mode, 0, 'OccBin: Piecewise Kalman Filter.');
        waitbar_indicator=1;
    else
        waitbar_indicator=0;
    end
...
while t<smpl
    ...
    if t>=first_period_occbin_update
        if waitbar_indicator
            [~, length_of_old_string] = wait_bar.run(t/smpl, hh_fig, sprintf('Period %u of %u', t,smpl),options_.console_mode,length_of_old_string);
        end
...
if (d==smpl)
    warning('missing_DiffuseKalmanSmootherH3_Z:: The diffuse filter never exited the diffuse state. There isn''t enough information to estimate the initial conditions of the nonstationary variables');
    return
end
