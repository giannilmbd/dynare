...
.. matcomm:: clean_current_folder ;
    ...
    previous runs of these files. Note that this also applies to the ``results`` subfolder. Proceed with caution.
...
.. matcomm:: internals FLAG ROUTINENAME[.m]|MODFILENAME
   ...
   MATLAB/Octave routine, or to extract some information about the
   state of Dynare.
   ...
   *Flags*
   ...
   ``--load-mh-history``
        ...
        Loads into the MATLAB/Octave’s workspace information
...
.. matcomm:: prior [OPTIONS[ ...]];
   ...
   *Options*
   ...
   .. option:: irfs(distribution)

      Computes the prior distribution of the impulse response functions
      of the endogenous variables (considering the linearized version
      of the model) by randomly sampling from the prior. The results will
      be stored in the ``prior`` subfolder in a
      ``_endogenous_variables_irfs_prior_draws.mat`` file.
   ...
   .. option:: simulate
      ...
      problems may arise when maximizing the posterior density and
...
.. matcomm:: dplot [OPTION VALUE[ ...]]
   ...
   *Options*
   ...
   .. option:: --range DATE1:DATE2
      ...
      This option is not mandatory and allows plotting the expressions
